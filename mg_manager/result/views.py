...
class ResultView(APIView):
    def post(self, request):
        data = request.data
        gr = GeneralResult(name=data['result'], input_objects=data['input_objects'], raw_res=json.dumps(data['raw_res']))
        gr.save()
    ...
    def post(self, request):
        ...
        for i, input in enumerate(res['input']):
            if input_objects[0] == 'MgSampleFile':
                input_obj = MgFile.objects.filter(
                ...
                try:
                    ProfileResult.objects.get(mg_file=input_obj)
                except:
                    res['input'].pop(i)
